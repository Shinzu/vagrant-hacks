            ...
            networks.each do |network|
              e_config = {}.tap do |entry|
                ...
                if network[:dns_servers]
                  dns_servers = {"addresses" => network[:dns_servers].split(",")}
                  dns_settings.merge!(dns_servers)
                end
                if network[:dns_search]
                  dns_search = {"search" => network[:dns_search].split(",")}
                  dns_settings.merge!(dns_search)
                end
                dns_settings.length > 0 && entry["nameservers"] = dns_settings
              end
